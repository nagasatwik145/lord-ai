...
export const getConversationsFn = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    console.error("[chat-history] getConversations:", err);
    return {
      conversations: [] as Array<{ id: string; title: string; updatedAt: string }>,
      error: err instanceof Error ? err.message : "Chat history unavailable",
    };
  }
});
...
export const getConversationFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: id }) => {
    try {
      ...
      console.error("[chat-history] getConversation:", err);
      return null;
    }
  });
...
export const createConversationFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      console.error("[chat-history] createConversation:", err);
    }
...
export const deleteConversationFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: id }) => {
    try {
      ...
      console.error("[chat-history] deleteConversation:", err);
    }
    return { id };
  });
...
export const updateConversationTitleFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      console.error("[chat-history] updateConversationTitle:", err);
    }
    return { id: data.id, title: data.title };
  });
