...
function SettingsPage() {
  ...
  const wipe = () => {
    ...
    Object.keys(localStorage).filter((k) => k.startsWith("lord:")).forEach((k) => localStorage.removeItem(k));
    location.reload();
  };
    ...
    <AppShell>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <HudPanel title="AI Model Defaults">
          <label className="text-xs uppercase tracking-wider text-muted-foreground">Default chat mode</label>
          <select value={defaultMode} onChange={(e) => setDefaultMode(e.target.value)} className="mt-1 w-full rounded-md border border-border/60 bg-background/40 px-3 py-2 text-sm outline-none">
        ...
        <HudPanel title="Theme">
          <p className="text-sm text-muted-foreground">LORD AI OS uses a single cinematic dark theme by design. Light mode is not supported — the HUD requires darkness.</p>
        </HudPanel>

        <HudPanel title="Data">
          <p className="text-sm text-muted-foreground">All data lives locally in your browser. Clear it to reset LORD.</p>
          <button onClick={wipe} className="mt-3 inline-flex items-center gap-2 rounded-md bg-destructive/15 border border-destructive/40 px-4 py-2 text-sm text-destructive hover:bg-destructive/25">
