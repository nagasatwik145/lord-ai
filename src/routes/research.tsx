...
function ResearchPage() {
  ...
  return (
    <AppShell>
      <h1 className="mb-1 font-display text-3xl tracking-wide gradient-text text-glow">Research Engine</h1>
      ...
      <HudPanel title="Query">
        <div className="flex flex-col gap-2 sm:flex-row">
          <div className="relative flex-1">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && run()} placeholder="Research topic…" className="w-full rounded-md border border-border/60 bg-background/40 pl-9 pr-3 py-2 text-sm outline-none focus:border-primary" />
          </div>
          <select value={depth} onChange={(e) => setDepth(e.target.value as "brief" | "deep")} className="rounded-md border border-border/60 bg-background/40 px-3 py-2 text-sm outline-none">
            <option value="brief">Brief</option>
            <option value="deep">Deep Dive</option>
          </select>
          <button onClick={run} disabled={busy || !query.trim()} className="inline-flex items-center gap-2 rounded-md bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground shadow-[0_0_18px_var(--hud)] disabled:opacity-40">
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <><BookOpen className="h-4 w-4" /> Investigate</>}
          </button>
        </div>
      </HudPanel>
