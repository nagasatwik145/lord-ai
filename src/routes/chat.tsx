          ...
          <div ref={scrollerRef} className="flex-1 overflow-y-auto rounded-xl hud-panel p-4 md:p-6">
            {messages.length === 0 ? (
              ...
              <ul className="space-y-4">
                ...
                {error && (
                  <li className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive space-y-1">
                    <div className="font-semibold">
                      Error: {error.message || "Connection failed"}
                    </div>
                    <div className="text-destructive/70">
                      {error.message?.includes("API") && "Check API configuration and try again."}
